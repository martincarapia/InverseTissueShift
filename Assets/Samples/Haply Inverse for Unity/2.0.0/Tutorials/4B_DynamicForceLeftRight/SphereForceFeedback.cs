...
namespace Haply.Samples.Tutorials._4B_DynamicForceLeftRight
{
    ...
    public class SphereForceFeedback : MonoBehaviour
    {
        ...
        private void OnDeviceStateChanged(Inverse3 device)
        {
            ...
            var force = ForceCalculation(device.Position, device.Velocity, sceneData.cursorRadii[index],
            ...
            {
                force += ForceCalculation(device.Position, device.Velocity, sceneData.cursorRadii[index],
                    inverse3s[otherIndex].Position, inverse3s[otherIndex].Velocity, sceneData.cursorRadii[otherIndex]);
            }
